...
fit_lasso <- function(data, nfolds = 10) {
  ...
  #   dummy variables
  X <- cbind(Intercept = 1, scale(model.matrix(~ . - 1, data = data)))#, 
  #           cat_var1_dummy, cat_var2_dummy)
...
fit_elnet <- function(data, nfolds = 10) {
  ...
  if (!is.null(cat_var2)) {
    data$cat_var1 <- as.factor(data$cat_var1)
  }
  
  if (!is.null(cat_var1)) {
    data$cat_var2 <- as.factor(data$cat_var2)
  }
...
# Function to fit a Spike and Slab prior model with CATEGORICAL variables 
#   using 'spikeslab' package and 
#   evaluate it on different datasets. It also plots the path of the 
#   estimates for the Spike and Slab model.
#
# INPUTS:
#     data               - A data frame containing the predictors and the response variable.
#                          The response variable should be named "y".
#     cat_var1           - Categorical variable. Binary.
#     cat_var2           - Categorical variable. 5 levels.
#     bigp_smalln        - A logical indicating if the high-dimensional low sample size adjustments
#                          should be made. Should be either TRUE or FALSE.
#     bigp_smalln_factor - A numeric adjustment factor to be used when bigp.smalln is TRUE.
#     seed               - An NEGATIVE integer used for setting the seed for reproducibility.
...
#     ss_results - The fitted Spike and Slab model.
#
fit_spikeslab_cat_prior <- function(data, cat_var1, cat_var2,
                                     bigp_smalln, bigp_smalln_factor = 0, screen = FALSE,
                                     K = 10, seed = -42) {
  ...
  # Remove the original categorical variables
  data <- subset(data, select = -c(cat_var1, cat_var2, y))
  
  # Convert categorical variables to one-hot encoding
  cat_var1_dummy <- model.matrix(~ 0 + as.factor(cat_var1))
  cat_var2_dummy <- model.matrix(~ 0 + as.factor(cat_var2))
  
  # Add the one-hot encoded categorical variables to the data frame, replacing the original variables
  x <- cbind(data, cat_var1_dummy, cat_var2_dummy)
...
# Function to fit the Spike-and-Slab LASSO model with some CATEGORICAL predictors, plot the coefficients,
# and extract selected variables from a given data frame.
#
# INPUTS:
#     data - Data frame where the first column is the response variable, and the rest are predictors.
#     cat_var1 and cat_var2 - categorical variables.
#     lambda1 - Slab variance parameter.
#     lambda0 - Vector of spike penalty parameters.
#     theta - Prior mixing proportion.
#     eps - Convergence criterion.
...
#                        ever selected along the regularization path.
#         plot - A plot of the coefficient paths for the fitted model.
fit_sslasso_cat <- function(data, cat_var1, cat_var2, lambda1 = 0.01, lambda0 = seq(1, nrow(data),), 
                        eps = 0.001, plot_width = 6, plot_height = 4) {
  # Input checks
  # Check that 'data' is a data frame
  if (!is.data.frame(data)) {
    stop("'data' must be a data frame.")
  ...
  # Add the one-hot encoded categorical variables to the data frame, replacing the original variables
  X <- as.matrix(cbind(cat_var1_dummy, cat_var2_dummy, data))
  ...
  # Fit the SSLASSO model
  result <- SSLASSO(X = X, y = y, penalty = "adaptive", variance = "unknown",
                    lambda1 = lambda1, lambda0 = lambda0, warn = TRUE, eps = eps)#, theta = theta)
